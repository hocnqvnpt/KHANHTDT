...
with nv100 as (	
				select ma_nv, LOAI, TEN_LOAIHD, MA_PB, TEN_PB, MA_TO, TEN_TO, ten_nv, ma_kh, NGAY_DKY_VI
                        , count(*) sl_nv
						, case when count(*) >= 1 then ceil( count(*) / 100 )
                          else -1
						  end sl
--                      , row_number() over(partition by MA_KH, a.MANV_RA_PCT, trunc(a.ngay_cn) order by a.ngay_cn) rnk
--				SELECT * 
...
with nv100 as (	
				select ma_nv, LOAI, TEN_LOAIHD, MA_PB, TEN_PB, MA_TO, TEN_TO, ten_nv, ma_kh, NGAY_DKY_VI, count(*) sl_nv
                        , count(*) sl_nv
						, case when count(*) >= 1 then ceil( count(*) / 100 )
                          else -1
						  end sl
...
--                        and TEN_LOAIHD not in ('DANG KY HUY CHUYEN DOI GOI CUOC', 'DOI SIM', 'DONG MO DV|0', 'DONG MO DV|1', 'DONG TRUOC GOI CUOC')					--DK theo file excel 2024-10
                      and TEN_LOAIHD not in ('HMM TRA SAU', 'DOI SIM', 'DANG KY HUY CHUYEN DOI GOI CUOC', 'DONG MO DV|0', 'DONG MO DV|1', 'DONG TRUOC GOI CUOC')  --DK theo file excel 2024-11
					  and exists (  Select 1 From ttkd_bct.cuoc_thuebao_ttkd where loaitb_id = 149 and ma_tb = a.ma_tb ) --DK theo file excel 2024-11
                group by LOAI, TEN_LOAIHD, a.MA_PB, a.TEN_PB, a.MA_TO, a.TEN_TO, a.ma_nv, a.ten_nv, nv.ten_vtcv
                order by LOAI, TEN_LOAIHD
		  )
select * From tbl1;
